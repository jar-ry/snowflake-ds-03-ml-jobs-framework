...
--   Compute Pool:  CHURN_MODEL_POOL_CPU
--   Stage:         MODELLING.PAYLOAD_STAGE
...
-- ─── Compute Pool (for ML Jobs / SPCS) ───────────────────────────────────────
USE ROLE ACCOUNTADMIN;

CREATE COMPUTE POOL IF NOT EXISTS CHURN_MODEL_POOL_CPU
    MIN_NODES          = 1
    MAX_NODES          = 3
...
GRANT USAGE, MONITOR ON COMPUTE POOL CHURN_MODEL_POOL_CPU
    TO ROLE CHURN_CLASSIFICATION_DEMO_ROLE;

-- ─── Internal Stage (for submit_directory payload) ───────────────────────────
USE ROLE CHURN_CLASSIFICATION_DEMO_ROLE;
USE DATABASE CHURN_CLASSIFICATION_DEMO;

CREATE STAGE IF NOT EXISTS MODELLING.PAYLOAD_STAGE
    ENCRYPTION = (TYPE = 'SNOWFLAKE_SSE');

-- ─── Grant privileges ────────────────────────────────────────────────────────
USE ROLE ACCOUNTADMIN;

-- Schema-level ownership (already owned via database ownership, but explicit for clarity)
...
-- ML Jobs / SPCS privileges
GRANT BIND SERVICE ENDPOINT ON ACCOUNT TO ROLE CHURN_CLASSIFICATION_DEMO_ROLE;
GRANT CREATE SERVICE ON SCHEMA MODELLING TO ROLE CHURN_CLASSIFICATION_DEMO_ROLE;
...
DESCRIBE COMPUTE POOL CHURN_MODEL_POOL_CPU;
SHOW STAGES IN SCHEMA MODELLING;
